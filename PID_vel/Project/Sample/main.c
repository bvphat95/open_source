...
int16_t err,pre_err,des_pulse,real_speed;
float Kp=2,Ki=0,Kd=0;//K=50,T=0.6
float pPart=0,iPart=0,dPart=0;
int16_t ctrl_speed=110;
...
void Motor_Control(int16_t des_Speed)
{
	pulse=TIM_GetCounter(TIM1)/4;
  TIM_SetCounter(TIM1,0);
	real_speed=(pulse*60)/(Ne*Ts);
	des_pulse=((des_Speed*Ne*Ts) /60); //chuyen toc do tu vong/phut sang pulse/s
	err=des_pulse-pulse;	
	pPart=Kp*err;
	...
	if(out>PWM_Period) out=PWM_Period-1;
	if(out<=0) out=1;
	TIM3->CCR1=out;
	pre_err=err;	
}
